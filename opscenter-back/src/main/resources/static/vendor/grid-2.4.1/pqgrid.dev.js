	...
	fnSB._createLayout = function() {
		...
		if (direction == "vertical") {
			...
			if (theme) {
				...
				ele.html(["<div class='top-btn pq-sb-btn'><span class='pq-col-sort-icon ui-icon ui-icon-triangle-1-n'>&nbsp;</span></div>", "<div class='pq-sb-slider'>", "<div class='vert-slider-top'></div>", "<div class='vert-slider-bg'></div>", "<div class='vert-slider-center'></div>", "<div class='vert-slider-bg'></div>", "<div class='vert-slider-bottom'></div>", "</div>", "<div class='bottom-btn pq-sb-btn'><span class='pq-col-sort-icon ui-icon ui-icon-triangle-1-s'>&nbsp;</span></div>"].join(""))
			}
		} else {
			...
			if (theme) {
				...
				ele.html(["<div class='left-btn pq-sb-btn'><span class='pq-col-sort-icon ui-icon ui-icon-triangle-1-w'>&nbsp;</span></div>", "<div class='pq-sb-slider pq-sb-slider-h'>", "<span class='horiz-slider-left'></span>", "<span class='horiz-slider-bg'></span>", "<span class='horiz-slider-center'></span>", "<span class='horiz-slider-bg'></span>", "<span class='horiz-slider-right'></span>", "</div>", "<div class='right-btn pq-sb-btn'><span class='pq-col-sort-icon ui-icon ui-icon-triangle-1-e'>&nbsp;</span></div>"].join(""))
			}
		}
	...
	fn.remoteRequest = function(objP) {
		...
		if (typeof DM.getUrl == "function") {
			...
			if (objURL && objURL.data) {
				dataURL = objURL.data
				//自定义表格功能@Create by Dennie 2018-11-27 
				if(dataURL.type=="autoGrid"){
					var sortingQS = this.getQueryStringSort();
					var filterQS = this.getQueryStringFilter();
					raiseFilterEvent = true;
					dataURL.pq_sort = sortingQS;
					dataURL.pq_curpage = PM.curPage;
					dataURL.pq_rpp = PM.rPP
					dataURL.pq_filter = filterQS;
				}
			}
		} else {
...
(function($) {
	...
	var cHeaderSearch = function(that) {
		...
		function filter(dataIndx, value, value2,refresh) {
			that.filter({
				data: [{
					dataIndx: dataIndx,
					value: value,
		...
		this.changeListener = {
			change: function(evt, ui) {
				console.info(ui.column.refresh)
				filter(ui.dataIndx, ui.value, ui.value2 ,ui.column.refresh)
			}
		};
		this.keyupListener = {
		...
		this.clickListener = {
			click: function(evt, ui) {
				filter(ui.dataIndx, ui.value,ui.column.refresh)
			}
		}
	};
	...
	fn.filter = function(objP) {
		var that = this,
			...
			DM = thisOptions.dataModel,
			FM = thisOptions.filterModel;
			refresh = (objP.data[0].refresh === undefined) ? true : objP.data[0].refresh;
		if (objP != undefined) {
