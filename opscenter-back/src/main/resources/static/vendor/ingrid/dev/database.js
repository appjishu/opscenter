...
pqgridView.formTarget = "#dataform";
pqgridView.titleTarget = "#formTile";
pqgridView.title = ["新增数据源","编辑数据源"];
pqgridView.url = {
		load : "/grid",
		add : "/database/createdatabase",
		edit : "/database/updatedatabase",
		del : "/database/deletedatabase"
}
pqgridView.gridSetting = {
	    ...
	    dataModel: {
	        ...
	        sortDir: ["up"],
	        getUrl: function(){
	            return { url: pqgridView.url.load, data: {type:"autoGrid",pq_method:"databaseList"} }
	        },
...
$(function () {
    ...
    $grid = $(pqgridView.target).pqGrid(pqgridView.gridSetting);
    $form = $(pqgridView.formTarget).my(pqgridView.manifest,pqgridView.row);
