var gongys = {
			  			...
			  			<vue-scroll v-bind="{ops:ops}">
	                            <el-card  v-for = "(cloudItem,index) in cloudlist" :key="index" >
	                                <div v-on:click='setCloud(cloudItem)'>
	                                	<span >
		                                    <p>{{cloudItem.gongysCode}} {{cloudItem.gongysName}} {{cloudItem.gongysAddress}}</p>
		                                </span>
	                                </div>
	                            </el-card>
	                        </vue-scroll>
    ...
    methods:{
        ...
        loadCloud:function(){
        	...
        	_this.loading = true;
        	var json =  {};
        	_this.cloudfilter.orgID = _this.row.orgID;
        	json = _this.copyParam();
        	json.poolName = "erp";
        	json.methodName = "PROC_SYS_GONGYSTANCHUANG_LOAD";
        	console.info(json)
        	$.ajax({
                type: "POST",
        ...
        setCloud: function(cloudItem){
        	var _this = this;
        	_this.$emit("setCloud",{opt:"add",type:"gongys",row:cloudItem})
        },
        clearCloud: function(cloudItem){
        	var _this = this;
        	_this.$emit("setCloud",{opt:"clear",type:"gongys"})
        },
    	closeCloud: function(cloudItem){
        	var _this = this;
        	_this.$emit("setCloud",{opt:"close",type:"gongys"})
        },
        //筛选框切换
